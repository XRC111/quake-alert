...
**Windows / Android / iOS** 三端。数据源为 **Wolfx**（CENC / 四川 / 重庆 / 台湾 CWA 多路 EEW 推送
+ 中国地震台网速报目录），达到阈值时触发**全屏强提醒弹窗 + 报警音 + 震动**，
并通过 GitHub Actions 在 `macos-latest` 上一次性产出三端安装包。

> ## ⚠️ 重要声明（请务必阅读）
>
> **本项目是纯技术演示与工程实践，用于学习 Kotlin Multiplatform / Compose Multiplatform 架构，
> 不构成任何形式的地震预警服务，也不面向公众发布、推送任何预警信息。**
>
> 1. **请通过官方渠道获取地震预警**：中国地震预警网官方小程序、各省地震局渠道、应急管理部门发布。
>    按《中华人民共和国防震减灾法》及多地方法规，地震预警信息由省级地震主管部门统一发布，
>    第三方须经技术评估并签订协议方可向公众播发（详见 README §9.5 关于 2026 年监管回应的记录）。
> 2. 本项目不对数据的及时性、准确性、完整性做任何担保，开发者不对因使用本项目造成的任何损失负责。
> 3. 本项目中的"测试预警"功能仅供开发者验证**本机**的通知/声音/震动链路，不代表任何真实预警。
> 4. 代码中的"本地估算"（烈度/倒计时）为工程近似值，仅作演示，**不可作为避险依据**。
