    ...
    AlertDialog(
        ...
        text = {
            Column(
                ...
                // ---- 烈度阈值（主） ----
                Text("所在地烈度阈值（中国烈度表，0~12 度）", style = MaterialTheme.typography.labelMedium)
                ...
                Text(
                    "报警由 API 下发的所在地预测烈度决定；低于此值不报警。",
                    fontSize = 11.sp,
                    color = MaterialTheme.colorScheme.outline,
                )

                // ---- 降级震级阈值 ----
                Text("降级震级阈值（API 未下发烈度时生效）", style = MaterialTheme.typography.labelMedium)
                Row(verticalAlignment = Alignment.CenterVertically) {
                    Slider(
                ...
                // ---- 去重窗口 ----
                Text("去重窗口（分钟）", style = MaterialTheme.typography.labelMedium)
                Row(verticalAlignment = Alignment.CenterVertically) {
                    Slider(
